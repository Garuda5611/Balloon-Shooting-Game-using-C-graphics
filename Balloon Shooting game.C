...
void main()
{
    ...
    draw_balloon(p,q);
    area=imagesize(p-4*bs,q-5*bs,p+4*bs,q+7*bs);
    balloon=malloc(area);
    getimage(p-4*bs,q-5*bs,p+4*bs,q+7*bs,balloon);
    putimage(p-4*bs, q-5*bs, balloon, COPY_PUT);
    draw_arrow(x ,y  );
    area = imagesize(x, y-as, x+(6*as), y+as);
    arrow=malloc(area);
    getimage(x, y-as, x+(6*as), y+as,arrow);
    putimage(x, y-as,arrow,XOR_PUT);
    draw_bow(x,y);
...
void draw_balloon(int x,int y)
{
	 setcolor(RED);
	 setfillstyle(1,RED);
	 fillellipse(x,y,3*bs,4*bs);
	 line(x,y+4*bs,x,y+6*bs);
}
...
{
	char cnt_arrow[5];
	putimage(*x, *y-as, arrow, COPY_PUT);
	delay(3);
	putimage(*x, *y-as, arrow, XOR_PUT);
	*x+=as;
...
void fly(int *x, int *y)
{
	int x1;
	char cnt_ball[5];
	putimage(*x-4*bs, *y-5*bs, balloon, COPY_PUT);
	delay(20);
	putimage(*x-4*bs, *y-5*bs, balloon, XOR_PUT);
	*y-=bs;
	if(*y<= 20)
	{
		*y=400;
...
void start()
{
	...
	outtextxy(10,270,"6. You require to score 60 points to enter the second level.");
